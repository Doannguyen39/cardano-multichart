...
def render_card(ticker, candles, err):
    ...
    <div style="background:#111318;border:1px solid #1a1d26;border-radius:6px;
        ...
        <div style="padding:5px 10px;border-bottom:1px solid #1a1d26;
            ...
            <span style="font-family:monospace;font-size:11px;font-weight:700;color:#e2e8f0">{ticker}</span>
            <span style="font-family:monospace;font-size:9px;color:#2d3748;margin-left:4px">/ADA</span>
            <span style="font-family:monospace;font-size:11px;color:#e2e8f0;margin-left:auto">{fmt_price(last["close"])}</span>
            <span style="font-family:monospace;font-size:10px;color:{cc};margin-left:8px;font-weight:600">{cs}</span>
        </div>
        <div id="c_{ticker}" style="flex:1;min-height:0"></div>
    </div>
    ...
    <script>
    (function(){{
        var el=document.getElementById('c_{ticker}');
        if(!el)return;
        ...
        var vs=chart.addHistogramSeries({{color:'#7c5cfc33',priceFormat:{{type:'volume'}},priceScaleId:'vol'}});
        chart.priceScale('vol').applyOptions({{scaleMargins:{{top:0.82,bottom:0}}}});
        var d={cdata};
        cs.setData(d.map(function(c){{return {{time:c.time,open:c.open,high:c.high,low:c.low,close:c.close}}}}}));
        vs.setData(d.map(function(c){{return {{time:c.time,value:c.volume,color:c.close>=c.open?'#00d4aa22':'#f03e3e22'}}}}}));
    ...
    """
    st.components.v1.html(html, height=264, scrolling=False)
